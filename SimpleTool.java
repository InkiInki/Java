...
public class SimpleTool {
	...
	public static void simpleToolIntroduction() {
		...
		if (tempI == 101) {
			System.out.println("小有所成，继续加油！");
		} else if (tempI - 1 == 1001) {
			System.out.println("现在才是真的开始！");
		} // Of if
	...
	public static double computeStandardDeviation(double[] paraList) {
		...
		double tempSum = 0;
		for (int i = 0; i < tempM; i++) {// 求和
			tempSum += paraList[i];
		} // Of for i

		double tempAve = tempSum / tempM;// 求平均值
		double tempVar = 0;
		for (int i = 0; i < tempM; i++) {// 求方差
	 ...
	 * Read file.
	 * 
	 * @param paraFileName The given file name(String).
	...
	public static String[][] fileRead(String paraFilename, String paraSplit) {
		...
		try (FileReader tempReader = new FileReader(paraFilename); 
				BufferedReader tempBr = new BufferedReader(tempReader) // 建立一个对象，它把文件内容转成计算机能读懂的语言
		) {
